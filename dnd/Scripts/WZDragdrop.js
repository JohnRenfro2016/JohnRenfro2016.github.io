...
dd.getWndW = function () {
  return dd.Int(
    (dd.db && !dd.op && !dd.w3c && dd.db.clientWidth) ? dd.db.clientWidth : (window.innerWidth || 0));
};
dd.getWndH = function () {
  return dd.Int(
    (dd.db && !dd.op && !dd.w3c && dd.db.clientHeight) ? dd.db.clientHeight : (window.innerHeight || 0));
};
...
dd.setCe = function (d_e, d_fnc) {
  d_fnc ? document.captureEvents(d_e) : document.removeEventListener(d_e);
};
...
dd.getEventTarget = function (d_e, d_s, d_n) {
  ...
  if (d_e && (d_s = d_e.target || d_e.srcElement || null) != null) {
    if (null != (d_n = d_s.id || d_s.name || null)) {
      if (d_n.indexOf("dIi15vNI1m6G") == d_n.length - 12) return dd.elements[d_n.substring(0, d_n.length - 12)] || null;
      if (d_n.indexOf("dIi15v") == d_n.length - 6) return dd.elements[d_n.substring(0, d_n.length - 6)] || null;
...
DDObj.prototype._setOpaRel = function (d_x, d_kd, d_y, d_o) {
  if (this.css && (this.diaphan || d_kd)) {
    ...
    else if (typeof this.css.filter != dd_u) this.css.filter = "Alpha(opacity=" + parseInt(100 * d_y) + ")";
...
dd.maxOffX = function (d_x, d_y) {
  return (
    (dd.obj.maxoffl + 1 && (d_y = dd.obj.defx - dd.obj.maxoffl) - d_x > 0 || dd.obj.maxoffr + 1 && (d_y = dd.obj.defx + dd.obj.maxoffr) - d_x < 0) ? d_y : d_x);
};
dd.maxOffY = function (d_x, d_y) {
  return (
    (dd.obj.maxofft + 1 && (d_y = dd.obj.defy - dd.obj.maxofft) - d_x > 0 || dd.obj.maxoffb + 1 && (d_y = dd.obj.defy + dd.obj.maxoffb) - d_x < 0) ? d_y : d_x);
};
dd.inWndW = function (d_x, d_y) {
  var d_wx = dd.getScrollX(),
    d_ww = dd.getWndW();
  return (
    ((d_y = d_wx + 2) - d_x > 0) || ((d_y = d_wx + d_ww + dd.obj.w - 2) - d_x < 0) ? d_y : d_x);
};
dd.inWndH = function (d_x, d_y) {
  var d_wy = dd.getScrollY(),
    d_wh = dd.getWndH();
  return (
    ((d_y = d_wy + 2) - d_x > 0) || ((d_y = d_wy + d_wh + dd.obj.h - 2) - d_x < 0) ? d_y : d_x);
};
// These two funcs limit the size of element when mouseresized.
// Implemented 22.5.2003 by Gregor Ltolf <gregor@milou.ch>, modified by Walter Zorn
dd.limW = function (d_w) {
  return (
    (dd.obj.minw - d_w > 0) ? dd.obj.minw : (dd.obj.maxw > 0 && dd.obj.maxw - d_w < 0) ? dd.obj.maxw : d_w);
};
dd.limH = function (d_h) {
  return (
    (dd.obj.minh - d_h > 0) ? dd.obj.minh : (dd.obj.maxh > 0 && dd.obj.maxh - d_h < 0) ? dd.obj.maxh : d_h);
};
...
function my_PickFunc() {
    menuIdNum = dd.obj.name.substring(3)
    menuIdNum++
    menuId = "AGE" + menuIdNum
    document.getElementById(menuId).style.left = (dd.obj.x / 1 + characterBoxWidth)
    document.getElementById(menuId).style.top = dd.obj.y
    document.getElementById(menuId).style.zIndex = dd.obj.z
  }
...
/*
     ...
     INTERNET ARCHIVE ON 22:12:08 Dec 08, 2023.
...
/*
playback timings (ms):
  captures_list: 206.865
  exclusion.robots: 0.065
  exclusion.robots.policy: 0.057
  cdx.remote: 0.054
  esindex: 0.008
  LoadShardBlock: 128.942 (3)
  PetaboxLoader3.datanode: 137.967 (4)
  load_resource: 51.789
  PetaboxLoader3.resolve: 24.16
*/